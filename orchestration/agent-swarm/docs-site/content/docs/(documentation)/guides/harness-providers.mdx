...
The runner reads `HARNESS_PROVIDER` in [`src/commands/runner.ts`](https://github.com/desplega-ai/agent-swarm/blob/main/src/commands/runner.ts) and calls the factory once per agent: `createProviderAdapter(process.env.HARNESS_PROVIDER || "claude")`.
...
- UI hook: `useTaskSessionLogs` in [`new-ui/src/api/hooks/use-tasks.ts`](https://github.com/desplega-ai/agent-swarm/blob/main/new-ui/src/api/hooks/use-tasks.ts), consumed at `new-ui/src/pages/tasks/[id]/page.tsx:42` and rendered by `<SessionLogViewer />` at `new-ui/src/components/shared/session-log-viewer.tsx`.
...
| Integrations UI (optional) | `new-ui/src/lib/integrations-catalog.ts` |
| Docs | `CLAUDE.md`, `README.md`, this guide, [Harness Configuration](/docs/guides/harness-configuration) |

**claude-managed reference files:** `src/providers/claude-managed-adapter.ts`, `src/providers/claude-managed-swarm-events.ts`, `src/providers/claude-managed-models.ts`, `src/commands/claude-managed-setup.ts`, `new-ui/src/lib/integrations-catalog.ts`.
